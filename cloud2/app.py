...
# Fallback approach: also inject a data-URL favicon.
# Why: some Streamlit Cloud deployments / browser caches can "pin" a favicon.
# This *may* help in certain cases; it won't hurt if the host ignores it.
def _inject_data_url_favicon(png_path: Path) -> tuple[bool, str]:
    try:
        if not png_path.exists():
            return False, f"Icon file not found: {png_path}"

        b64 = base64.b64encode(png_path.read_bytes()).decode("utf-8")

        # Note: Streamlit renders markdown into the body, not <head>.
        # Some browsers still accept these <link rel=icon> tags from body; some don't.
        st.markdown(
            f"""
        ...
        return True, "Injected data-URL favicon tags"
...
try:
    ...
    if hasattr(st, "query_params"):
        debug_flag = str(st.query_params.get("debug_favicon", "0")).strip() in ("1", "true", "True", "yes", "on")
except Exception:
    debug_flag = False

ok, msg = _inject_data_url_favicon(ICON_PATH)
if debug_flag:
    ...
    st.sidebar.write("Injection:", ok, msg)
