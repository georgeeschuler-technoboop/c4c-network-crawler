...
# Region Selector UI
# =============================================================================
def region_selector_ui(project_id: str = None, *, use_card: bool = False) -> dict:
    """
    ...
    Args:
        ...
        use_card: If True, render inside a C4C sub-card (pipeline layout).
    ...
    # ---- Card wrapper / header ----
    if use_card:
        st.markdown('<div class="c4c-subcard">', unsafe_allow_html=True)
        st.markdown(
            '<div class="c4c-subcard-hd">'
            '🗺️ Regional Perspective'
            '<span class="c4c-subcard-sub">Optional</span>'
            '</div>',
            unsafe_allow_html=True,
        )
        st.markdown('<div class="c4c-subcard-body">', unsafe_allow_html=True)
    ...
    # ---- Safe config update (handles older project_store signatures) ----
    def _safe_update_region(cfg_in: dict, **kwargs) -> dict:
        try:
            return update_region_in_config(cfg_in, **kwargs)
        except TypeError:
            # Backward-compatible fallback: update cfg["region_filter"] directly
            cfg_local = dict(cfg_in) if cfg_in is not None else {}
            rf_local = dict(cfg_local.get("region_filter", {}) or {})
            mode = kwargs.get("mode", rf_local.get("mode", "off"))
            rf_local["mode"] = mode
            if "preset_id" in kwargs:
                rf_local["preset_id"] = kwargs.get("preset_id")
                rf_local["custom_admin1_codes"] = []
            if "custom_admin1_codes" in kwargs:
                rf_local["custom_admin1_codes"] = kwargs.get("custom_admin1_codes") or []
                rf_local["preset_id"] = "none"
            cfg_local["region_filter"] = rf_local
            return cfg_local
    ...
    # Persist mode when changed
    if mode != current_mode:
        cfg = _safe_update_region(cfg, mode=mode)
    ...
    # ---- Preset mode ----
    if mode == "preset":
        preset_ids = list(REGION_PRESETS.keys())
        current_preset_id = rf.get("preset_id", "none")
        ...
        preset_id = st.selectbox(
            ...
            format_func=lambda x: REGION_PRESETS.get(x, {}).get("name", x),
            key=f"preset_region_{project_id}",
        )

        if preset_id != current_preset_id:
            cfg = _safe_update_region(cfg, mode="preset", preset_id=preset_id)
            save_project_config(project_id, cfg)

        selected = REGION_PRESETS.get(preset_id, REGION_PRESETS["none"])
        if use_card:
            st.markdown("</div></div>", unsafe_allow_html=True)
        return selected
    ...
    current_codes = rf.get("custom_admin1_codes", []) or []
    current_us = [s for s in current_codes if s in US_STATES]
    current_ca = [s for s in current_codes if s in CA_PROVINCES]
    ...
    region_def = {
        ...
        "include_us_states": list(us_states),
        "include_ca_provinces": list(ca_provinces),
...
# Upload Interface
# =============================================================================
def render_upload_interface(project_name: str, *, uploaded_files=None, tax_year_override:str="", hide_upload_controls: bool=False, pipeline_mode: bool=False):
    ...
    if st.session_state.processed:
        ...
        if (not hide_upload_controls) and (not pipeline_mode):
        ...
        if hide_upload_controls:
            # Uploads are collected in Stage 2 and passed here
            uploaded_files = uploaded_files or []
            tax_year_override = tax_year_override or ""
            ...
            uploaded_files = st.file_uploader(
                ...
                key=f"stage2_uploads_{project_name}"
            )
            ...
            tax_year_override = st.text_input(
                ...
                key=f"stage2_tax_year_{project_name}"
            )

            st.divider()

# Region selector (after upload, before processing)
        region_def = region_selector_ui(project_id=project_name, use_card=True)
        st.session_state['region_def'] = region_def
...
def main():
    ...
    # ==========================================================================
    current_project = st.session_state.get("current_project")
    uploads_key = f"stage2_uploads_{current_project}" if current_project else None
    tax_key = f"stage2_tax_year_{current_project}" if current_project else None

    # Read current Stage 2 inputs (if already entered)
    stage2_uploaded_files = st.session_state.get(uploads_key, []) if uploads_key else []
    stage2_tax_year = st.session_state.get(tax_key, "") if tax_key else ""

    stage2_status = "idle" if not current_project else ("complete" if (stage2_uploaded_files and len(stage2_uploaded_files) > 0) else "active")
    c4c_stage_open(
        2,
        "Data",
        status=stage2_status,
        subtitle="Upload IRS 990 filings (PDF/XML) for this project. This stage only collects files; processing happens in Stage 3."
    )

    if not current_project:
        st.info("Complete **Stage 1 · Project** to choose or create a project before uploading data.")
    else:
        st.caption("📱 iPad tip: long-press the ProPublica 'XML' link → 'Download Linked File'. Tapping and saving may produce a non-XML text file.")

        with st.expander("📚 Data source guide (recommended reading)", expanded=False):
            st.markdown("""
            **Best practice:** Use **ProPublica XML** whenever possible (most accurate).

            | Source | Format | Notes |
            |---|---|---|
            | **ProPublica XML** | `.xml` | ⭐⭐⭐ Best choice — includes structured grantee EINs |
            | **ProPublica PDF** | `.pdf` | ⭐⭐ Beta — works well but parsing may vary slightly |
            | **IRS direct PDF** | `.pdf` | ⭐⭐ Not yet optimized |

            **How to get XML files:**
            1. Go to ProPublica Nonprofit Explorer
            2. Search by foundation name or EIN
            3. Choose a filing year
            4. Download the **XML** (not the PDF)

            **Supported types:**
            - **990-PF** (private foundations): PDF or XML
            - **990 + Schedule I** (public charities making grants): XML only
            """)
        ...
        stage2_uploaded_files = st.session_state.get(uploads_key, []) if uploads_key else []
        if stage2_uploaded_files:
            st.success(f"✅ **{len(stage2_uploaded_files)} file(s) ready** for processing in Stage 3.")
    ...
    stage3_ready = bool(current_project) and (bool(stage2_uploaded_files) or processed)
    stage3_status = "idle" if not stage3_ready else ("complete" if processed else "active")

    c4c_stage_open(
        3,
        "Process",
        status=stage3_status,
        subtitle="Select region options and parse filings to produce nodes, edges, grant details, and diagnostics."
    )

    if not current_project:
        st.info("Complete **Stage 1 · Project** first.")
    elif not stage2_uploaded_files and not processed:
        st.info("Upload files in **Stage 2 · Data** to enable processing.")
    else:
        # Run the existing processing & results UI, but hide upload controls (handled in Stage 2)
        render_upload_interface(
            current_project,
            uploaded_files=stage2_uploaded_files,
            tax_year_override=st.session_state.get(tax_key, "") if tax_key else "",
            hide_upload_controls=True,
            pipeline_mode=True
        )
    ...
    c4c_stage_open(
        ...
        subtitle="Export a ZIP bundle for Polinode + InsightGraph, and optionally save the bundle to Project or Cloud."
    )
    ...
    if not processed:
        ...
        render_downloads(
            ...
            region_def=region_def,
            board_detail_df=getattr(st.session_state, "board_detail_df", None),
        )
