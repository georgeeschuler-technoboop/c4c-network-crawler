...
def c4c_header(title: str, subtitle: str = "", icon_url: Optional[str] = None, right_html: str = "") -> None:
    ...
    """
    logo_html = (
        f"<img src='{{escape(icon_url)}}' style='width:42px;height:42px;object-fit:contain;'/>"
        if icon_url else ""
    )
    st.markdown(
        f"""
...
def c4c_badge(text: str, tone: str = "default") -> str:
    """
    Return a badge HTML string. tone in: default, indigo, amber, terracotta, teal, success, warn, danger.
    """
    t = tone.strip().lower()
