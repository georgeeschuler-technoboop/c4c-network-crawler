...
def c4c_header(title: str, subtitle: str = ""):
    logo_html = f"<img src='{APP_ICON_URL}' style='width:42px;height:42px;object-fit:contain;'/>" if APP_ICON_URL else ""
    st.markdown(
        f"""
...
def c4c_card_open(title: str, subtitle: str = "", variant: str = ""):
    cls = "c4c-card" + (f" {variant}" if variant else "")
    st.markdown(
        f"""
<div class='{cls}'>
...
def c4c_console(title: str, lines: list):
    safe = "\n".join(lines)
    st.markdown(
        f"""
