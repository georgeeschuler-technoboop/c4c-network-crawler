...
def compute_sector_analysis(nodes_df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        'is_concentrated': top_pct > SECTOR_CONCENTRATION_THRESHOLD,
        'is_diverse': len(sector_counts) >= SECTOR_DIVERSITY_MIN and top_pct < 30
    }
...
class SocialAnalyzer(NetworkAnalyzer):
    ...
    def _generate_project_summary(self) -> ProjectSummary:
        ...
        if 'node_type' in self.nodes_df.columns:
            company_count = len(self.nodes_df[self.nodes_df['node_type'].str.lower() == 'company'])
        ...
        if 'source_type' in self.nodes_df.columns:
            seed_count = len(self.nodes_df[self.nodes_df['source_type'].str.lower() == 'seed'])
        ...
        return ProjectSummary(
            ...
            edge_counts={
                ...
                "connections": len(self.edges_df)  # All edges are connections in social networks
            },
