...
try:
    # Try absolute import (works when running via Streamlit)
    ...
    try:
        # Fallback: relative import (works when running as package)
...
SOCIAL_ANALYZER_VERSION = "1.1.8"
ANALYZER_BUILD = "v1.1.8-2026-01-06-abs-import"
...
class SocialAnalyzer(NetworkAnalyzer):
    ...
    def _generate_insight_cards(self, health: HealthScore, component_stats: dict) -> list[InsightCard]:
        ...
        cards.append(InsightCard(
            ...
            evidence={"metrics": component_stats}
        ))
        ...
        if self.sector_analysis.get('available'):
            cards.append(InsightCard(
                ...
                evidence={"metrics": self.sector_analysis}
            ))
    ...
    def _generate_markdown_report(self, health: HealthScore, cards: list[InsightCard], 
        ...
        lines.append(f"**Network Type:** Social Network ({self.source_app})")
        ...
        lines.append(f"*Report generated by C4C InsightGraph — Social Network Analyzer ({copy_status})*")
    ...
    def _generate_project_summary(self) -> ProjectSummary:
        ...
        if 'is_seed' in self.nodes_df.columns:
            seed_count = int(self.nodes_df['is_seed'].sum())
        ...
        return ProjectSummary(
            ...
            sectors=self.sector_analysis if self.sector_analysis and self.sector_analysis.get('available') else None
        )
