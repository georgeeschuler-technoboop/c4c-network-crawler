...
def compute_flow_stats(edges_df: pd.DataFrame, metrics_df: pd.DataFrame) -> dict:
    ...
    total_funding = grant_edges['amount'].sum()
    funders = grant_edges['from_id'].nunique()
    grantees = grant_edges['to_id'].nunique()
    
    # Top 5 share
    funder_totals = grant_edges.groupby('from_id')['amount'].sum().sort_values(ascending=False)
    top_5_share = funder_totals.head(5).sum() / total_funding * 100 if total_funding > 0 else 0
    
    # Multi-funder grantees
    grantee_funder_counts = grant_edges.groupby('to_id')['from_id'].nunique()
    multi_funder = (grantee_funder_counts > 1).sum()
    multi_funder_pct = multi_funder / grantees * 100 if grantees > 0 else 0
...
class FunderAnalyzer(NetworkAnalyzer):
    ...
    def _generate_project_summary(self) -> ProjectSummary:
        ...
        org_count = len(self.nodes_df[self.nodes_df['node_type'].str.lower().isin(['org', 'organization'])])
        person_count = len(self.nodes_df[self.nodes_df['node_type'].str.lower() == 'person'])
        
        grant_count = len(self.edges_df[self.edges_df['edge_type'].str.lower() == 'grant'])
        board_count = len(self.edges_df[self.edges_df['edge_type'].str.lower().isin(['board', 'board_membership'])])
        ...
        return ProjectSummary(
            ...
            funding={
                "total_amount": self.flow_stats['total_funding'],
                "funder_count": self.flow_stats['funder_count'],
                "grantee_count": self.flow_stats['grantee_count'],
                "top_5_share": self.flow_stats['top_5_funders_share']
            },
            governance={
